...
export default function HorizontalTemplate({ data }) {
  ...
  return (
    <div id="resume-preview" style={container}>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: '#c9a84c', fontWeight: 600, letterSpacing: 0.3 }}>
            {personalInfo.jobTitle || 'Professional Title'}
          </div>
        </div>

        {/* Right — Contact details stacked */}
        <div style={{ textAlign: 'right', fontSize: 10, color: '#374151', lineHeight: 1.9, flexShrink: 0 }}>
      ...
      {/* ── BODY — single full-width column (no sidebar) ── */}
      <div style={{ padding: '22px 36px' }}>
        ...
        {personalInfo.summary && (
          <div style={{ marginBottom: 18 }}>
            <div style={sectionTitle}>Professional Summary</div>
            <p style={{ fontSize: 10.5, color: '#374151', lineHeight: 1.75, margin: 0 }}>{personalInfo.summary}</p>
          </div>
        )}
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            {experience.map((exp, i) => exp.role && (
              <div key={i} style={{ marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ fontWeight: 700, fontSize: 11.5, color: '#1a1f36' }}>{exp.role}</div>
                  <div style={{ fontSize: 10, color: '#6b7280', flexShrink: 0, marginLeft: 12 }}>
                    {exp.startDate}{exp.endDate ? ` - ${exp.endDate}` : ''}
                  </div>
                </div>
                <div style={{ color: '#c9a84c', fontWeight: 600, fontSize: 10.5, marginBottom: 4 }}>{exp.company}</div>
                <div style={{ fontSize: 10.5, color: '#374151', lineHeight: 1.7, whiteSpace: 'pre-line' }}>{exp.description}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {projects.some(p => p.name) && (
          <div style={{ marginBottom: 18 }}>
            ...
            {projects.map((proj, i) => proj.name && (
              <div key={i} style={{ marginBottom: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ fontWeight: 700, fontSize: 11.5, color: '#1a1f36' }}>
                    {proj.name}{proj.link ? ` | ${proj.link}` : ''}
                  </div>
                </div>
                {proj.techStack && (
                  <div style={{ color: '#c9a84c', fontWeight: 600, fontSize: 10.5, marginBottom: 4 }}>{proj.techStack}</div>
                )}
                <div style={{ fontSize: 10.5, color: '#374151', lineHeight: 1.7 }}>{proj.description}</div>
              </div>
            ))}
          </div>
        )}
